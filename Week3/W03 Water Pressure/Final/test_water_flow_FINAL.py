from waterflow import water_column_height, pressure_gain_from_water_height, pressure_loss_from_pipe, \
    pressure_loss_from_fittings, reynolds_number, pressure_loss_from_pipe_reduction, convert_to_psi
...
def test_pressure_gain_loss_from_pipe():
...
def test_convert_to_psi():
    assert convert_to_psi(100) == approx(14.5038, abs=0.0001)
    assert convert_to_psi(200) == approx(29.0076, abs=0.0001)
    assert convert_to_psi(300) == approx(43.5113, abs=0.0001)
    assert convert_to_psi(500) == approx(72.5189, abs=0.0001)
