def main():
    ...
    if vin in vehicles_dict:
        print(f"The vehicle with VIN {vin} is a ", end="")

        # Find the data for the vehicle that the user wants.
        for vehicle in vehicles_dict:
            if vehicle == vin:
                vehicle_data = vehicles_dict[vehicle] # type: ignore
                print(f"{vehicle_data[MANUFACTURER_INDEX]} "
                      f"{vehicle_data[MODEL_INDEX]} "
                      f"{vehicle_data[COLOR_INDEX]}.")
